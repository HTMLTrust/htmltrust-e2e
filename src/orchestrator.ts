...
async function main(): Promise<void> {
  ...
  const extDir = path.resolve(E2E_DIR, "../htmltrust-browser-reference");
  await execFileAsync("npm", ["run", "build:chrome"], { cwd: extDir, timeout: 60_000 });
  const extensionPath = path.join(extDir, "build/chromium");
